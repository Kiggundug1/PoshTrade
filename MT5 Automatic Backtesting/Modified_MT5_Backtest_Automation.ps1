...
function Validate-Configuration {
    ...
    # Validate paths
    foreach ($pathKey in @("batchFilePath", "configIniPath", "reportPath", "logFilePath", "errorScreenshotsPath", "checkpointFile", "configFilePath", "performanceHistoryFile")) {
        if ([string]::IsNullOrEmpty($config[$pathKey])) {
            $validationErrors += "Configuration error: $pathKey cannot be empty"
        }
    }
    
    # Validate numeric values
    foreach ($numKey in @("maxWaitTimeForTest", "maxRetries", "maxConsecutiveFailures", "retryBackoffMultiplier", "maxRetryWaitTime", "maxLogSizeMB", "maxLogBackups")) {
        if ($config[$numKey] -le 0) {
            $validationErrors += "Configuration error: $numKey must be greater than 0"
        }
    }
    
    # Validate boolean values
    foreach ($boolKey in @("skipOnError", "autoRestartOnFailure", "adaptiveWaitEnabled", "verboseLogging")) {
        if ($config[$boolKey] -isnot [bool]) {
            $validationErrors += "Configuration error: $boolKey must be a boolean value"
        }
    }
...
function Write-Log {
    ...
    if ((Test-Path -Path $config.logFilePath) -and 
        ((Get-Item -Path $config.logFilePath).Length / 1MB) -gt $config.maxLogSizeMB) {
...
function Rotate-LogFiles {
    ...
    try {
        ...
        for ($i = $config.maxLogBackups; $i -gt 0; $i--) {
            $currentBackup = "$($config.logFilePath).$i"
            if ($i -eq $config.maxLogBackups) {
                ...
                $nextBackup = "$($config.logFilePath).$($i+1)"
                if (Test-Path -Path $currentBackup) {
        ...
        if (Test-Path -Path $config.logFilePath) {
            Move-Item -Path $config.logFilePath -Destination "$($config.logFilePath).1" -Force
        }
        
        Write-Host "Log file rotated due to size limit ($($config.maxLogSizeMB) MB)" -ForegroundColor Yellow
    }
    catch {
        Write-Host "Error rotating log files: $($_.Exception.Message)" -ForegroundColor Red
    }
}
...
function Start-MT5WithBatchFile {
    ...
    try {
        ...
        # Run the batch file
        $process = Start-Process -FilePath $config.batchFilePath -PassThru
...
function Set-StrategyTesterFocus {
    ...
    for ($attempt = 1; $attempt -le $maxAttempts; $attempt++) {
        if (Test-WindowExists -WindowTitle $constants.WINDOW_STRATEGY_TESTER) {
            $focusSet = Set-WindowFocus -WindowTitle $constants.WINDOW_STRATEGY_TESTER
            if ($focusSet) {
                break
            }
        }
        elseif (Test-WindowExists -WindowTitle $constants.WINDOW_TESTER) {
            $focusSet = Set-WindowFocus -WindowTitle $constants.WINDOW_TESTER
            if ($focusSet) {
                break
            }
        }
        ...
        while (-not $testCompleted) {
            ...
            if ($noProgressCounter -ge $constants.MAX_NO_PROGRESS_INTERVALS) {
                Write-Log -Level "ERROR" -Message "Backtest appears to be stuck at $lastProgressValue%. Attempting recovery..." -Details @{
                    ...
                    "maxAllowedIntervals" = $constants.MAX_NO_PROGRESS_INTERVALS
                }
            ...
            if ($testWaitTime -gt $config.maxWaitTimeForTest * 10) {
                Write-Log -Level "ERROR" -Message "Maximum wait time exceeded. Forcing test completion." -Details @{
                    "maxWaitTime" = $config.maxWaitTimeForTest * 10
                    "elapsedTime" = $testWaitTime
                }
...
function Update-TestProgress {
    ...
    try {
        ...
        Write-Log -Level "DEBUG" -Message "Error updating test progress: $($_.Exception.Message)" -Details @{}
    }
}
...
function Log-ProgressInformation {
    ...
    if ($Progress -gt 0) {
        $elapsedTime = [int](Get-Date).ToFileTime() - $script:runtime.testStartTime
        ...
        $currentTime = [int](Get-Date).ToFileTime()
        if ($currentTime - $script:runtime.lastProgressLogTime -gt 300 -or 
            ($Progress % $config.logProgressInterval -eq 0 -and $Progress -ne $script:runtime.previousLoggedProgress)) {
            ...
            $script:runtime.previousLoggedProgress = $Progress
            $script:runtime.lastProgressLogTime = $currentTime
        }
    }
}
    ...
    try {
        ...
        if ($MtFrozenCounter.Value -ge 3) {
            Write-Log -Level "ERROR" -Message "MT5 appears to be frozen. Attempting recovery..." -Details @{
                "frozenCounter" = $MtFrozenCounter.Value
            }
...
function Save-ReportUsingContextMenu {
    ...
    try {
        # Ensure Strategy Tester window is active
        if (-not (Set-StrategyTesterFocus)) {
            Write-Log -Level "DEBUG" -Message "Could not set focus to Strategy Tester window" -Details @{}
            return $false
        }
        ...
        Write-Log -Level "DEBUG" -Message "Error saving report using context menu: $($_.Exception.Message)" -Details @{}
                return $false
    }
}
...
function Save-ReportUsingMenuBar {
    ...
    try {
        ...
        Write-Log -Level "DEBUG" -Message "Error saving report using menu bar: $($_.Exception.Message)" -Details @{}
        return $false
    }
}
...
function Handle-SaveAsDialog {
    ...
    try {
        ...
        if (-not (Wait-ForWindow -WindowTitle $constants.WINDOW_SAVE_AS -MaxWaitTime 10)) {
            Write-Log -Level "WARN" -Message "Save As dialog did not appear" -Details @{}
            return $false
        }
        
        # Set focus to Save As dialog
        if (-not (Set-WindowFocus -WindowTitle $constants.WINDOW_SAVE_AS)) {
            Write-Log -Level "WARN" -Message "Could not set focus to Save As dialog" -Details @{}
            return $false
        }
        ...
        # Handle potential overwrite confirmation
        if (Test-WindowExists -WindowTitle $constants.WINDOW_CONFIRM) {
            if (-not (Set-WindowFocus -WindowTitle $constants.WINDOW_CONFIRM)) {
                Write-Log -Level "WARN" -Message "Could not set focus to confirmation dialog" -Details @{}
            }
            [System.Windows.Forms.SendKeys]::SendWait("y")
            Invoke-AdaptiveWait -WaitTime $constants.WAIT_MEDIUM
        }
        ...
        if (-not (Test-Path -Path $fullPath)) {
            Write-Log -Level "WARN" -Message "Report file was not created at expected location" -Details @{
                "path" = $fullPath
            }
            ...
            if (-not (Test-Path -Path $fullPath)) {
                Write-Log -Level "ERROR" -Message "Report file was not created even after waiting" -Details @{
                    "path" = $fullPath
                }
                return $false
            }
        ...
        Write-Log -Level "ERROR" -Message "Error handling Save As dialog: $($_.Exception.Message)" -Details @{
            "stackTrace" = $_.ScriptStackTrace
        }
        return $false
    }
...
function Invoke-CleanupAfterFailedSave {
    ...
    try {
        ...
        Write-Log -Level "INFO" -Message "Performed cleanup after failed save" -Details @{}
    }
    catch {
        # Ignore errors when closing
        Write-Log -Level "DEBUG" -Message "Error during cleanup after failed save: $($_.Exception.Message)" -Details @{}
    }
}
...
function Restart-MT5 {
    ...
    Write-Log -Level "WARN" -Message "Detected $($script:runtime.consecutiveFailures) consecutive failures. Restarting MT5..." -Details @{
        "consecutiveFailures" = $script:runtime.consecutiveFailures
    ...
    $mt5Processes = Get-Process -Name $constants.PROCESS_MT5 -ErrorAction SilentlyContinue
    if ($mt5Processes) {
        try {
            Stop-Process -Name $constants.PROCESS_MT5 -Force -ErrorAction SilentlyContinue
            Write-Log -Level "INFO" -Message "Forcefully terminated MT5 processes" -Details @{
                "processCount" = $mt5Processes.Count
            }
        }
        catch {
            Write-Log -Level "WARN" -Message "Failed to forcefully terminate MT5 processes: $($_.Exception.Message)" -Details @{}
        }
    }
...
function Validate-Configuration {
    ...
    if (-not (Test-Path -Path $config.batchFilePath)) {
        $configErrors += "Batch file does not exist: $($config.batchFilePath)"
    }
    
    if (-not (Test-Path -Path $config.configIniPath)) {
        $configErrors += "Config INI file does not exist: $($config.configIniPath)"
    }
    
    # Check for valid numeric values
    if ($config.maxWaitTimeForTest -le 0) {
        $configErrors += "maxWaitTimeForTest must be greater than 0"
    }
    
    if ($config.maxRetries -lt 0) {
...
# Add valid symbols and timeframes to constants
$script:constants["VALID_SYMBOLS"] = @(
    "EURUSD", "GBPUSD", "USDJPY", "AUDUSD", "USDCHF", "EURGBP", 
    "EURJPY", "EURCHF", "AUDJPY", "NZDUSD", "USDCAD", "GBPJPY"
)

$script:constants["VALID_TIMEFRAMES"] = @(
    "M1", "M5", "M15", "M30", "H1", "H4", "D1", "W1", "MN1"
)

$script:constants["SYSTEM_CHECK_INTERVAL"] = 30
